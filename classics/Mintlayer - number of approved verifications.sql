...
WHERE
	organizations.stripe_customer_id = 'cus_KQzdfoR14RnCkU'
    ...
    -- DEFINE TIMEFRAME WITH IS
    AND TO_CHAR(identity_accesses.completed_at, 'YYYY-MM-DD') >= '2022-01-01' -- GOES BY UTC TIME
    AND TO_CHAR(identity_accesses.completed_at, 'YYYY-MM-DD') < '2022-11-16' -- GOES BY UTC TIME
GROUP BY
	organizations.id,
	organizations.name,
